...
import torch
import torchvision
from models import inception_v3, resnet18, resnet101, densenet121, densenet169, resnet50, mobilenet_v2, SENet18
...
def Inception(num_classes, test=False):
    model = inception_v3()
    if not test:
        if LOCAL_PRETRAINED['inception'] == None:
            state_dict = load_state_dict_from_url(model_urls['inception'], progress=True)
        else:
            state_dict = state_dict = torch.load(LOCAL_PRETRAINED['inception'])
...
def Resnext101_32x8d(num_classes, test=False):
    ...
    if not test:
        if LOCAL_PRETRAINED['resnext101_32x8d'] == None:
            ...
            state_dict = torch.load(LOCAL_PRETRAINED['resnext101_32x8d'])
        model.load_state_dict(state_dict)
    fc_features = model.fc.in_features
    model.fc = nn.Linear(fc_features, num_classes)
    return model
...
def se_net(num_classes, test = False):
    model = SENet18()
    # if not test:
        # if LOCAL_PRETRAINED['resnet50'] == None:
            # state_dict = load_state_dict_from_url(model_urls['resnet50'], progress=True)
